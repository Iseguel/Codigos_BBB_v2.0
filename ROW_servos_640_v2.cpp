...
{   
    ...
    float dWidth = cap.get(CV_CAP_PROP_FRAME_WIDTH); //get the width of frames of the video
    float dHeight = cap.get(CV_CAP_PROP_FRAME_HEIGHT); //get the height of frames of the video
  ...
  while(true)
  {
    ...
    Point seed = Point(320,470);  //640x480
    ...
    cont++;
    //Dibuja una linea desde el "punto base", centro inferior de la imagen hasta el CM 
    arrowedLine(src, Point(320,480),Point(mcFx,mcFy), Scalar(0,255,0),2, 8, 0, 0.1) ;
    ...
    putText(src, texto_mc, Point(mcFx,mcFy), FONT_HERSHEY_PLAIN, 1, Scalar(0,255,0),1,1);
...
float VectorDir(){
        ...
        //Modulo = Distancia(160, 240, mcF.x, mcF.y); //320x240
        //Angulo = CalcAngulo(Modulo, (240 - mcF.y), mcF.x);
